...
God.init = function() {
  ...
  const masterArgIndex = process.argv.indexOf('--master') + 1
  const masterArg = masterArgIndex && process.argv[masterArgIndex]
  const masterPath = masterArg && path.resolve(process.cwd(), masterArg)

  if (!!masterPath) {
    if (ProcessUtils.isESModule(masterPath) === true) {
      import(url.pathToFileURL(masterPath))
    }
    else
      require('module')._load(masterPath, null, true)
  }
}
